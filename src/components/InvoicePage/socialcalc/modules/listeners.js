...
export function setupMouseListener() {
  ...
  SocialCalc.ProcessEditorMouseDown = function (e) {
    ...
    var isToggleCell = false;
    if (SocialCalc.Callbacks && SocialCalc.Callbacks.ToggleCell) {
      // Check if this cell is a toggle-cell (tc) constraint before calling
      if (SocialCalc.EditableCells && SocialCalc.EditableCells.constraints) {
        var control = SocialCalc.GetCurrentWorkBookControl && SocialCalc.GetCurrentWorkBookControl();
        if (control && control.workbook) {
          var sheetid = control.currentSheetButton ? control.currentSheetButton.id : null;
          var sheetobj = sheetid ? control.workbook.sheetArr[sheetid] && control.workbook.sheetArr[sheetid].sheet : null;
          var sheetname = sheetobj ? sheetobj.sheetname : null;
          var cellConstraint = sheetname ? SocialCalc.EditableCells.constraints[sheetname + '!' + result.coord] : null;
          if (cellConstraint && cellConstraint[0] === 'tc') {
            isToggleCell = true;
          }
        }
      }
    ...
    // --- CUSTOM LOGIC ---
    // Skip the text edit modal if this cell is a toggle (checkmark) cell
    if (!isToggleCell) {
