...
async function run() {
  ...
  for (const [key, dev] of Object.entries(devicesToCapture)) {
    ...
    try {
      ...
      await page.waitForSelector('.footer-type-btn', { state: 'visible', timeout: 10000 });
      // Switch to the Check-up tab (index 1) to ensure we are on a sheet with editable cells
      console.log(`- Switching to Check-up tab to find editable cells...`);
      await page.locator('.footer-type-btn').nth(1).click();
      await page.waitForTimeout(2000); // Allow sheet rendering to settle
